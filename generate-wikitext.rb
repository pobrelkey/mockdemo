...
This is a brief comparison of several major [mock object](http://www.mockobjects.com/) libraries available for Java today.
...
This article was inspired by Jean Tessier's ["Mocking in Java: jMock vs. EasyMock"](http://jeantessier.com/SoftwareEngineering/Mocking.html).


# Disclaimer

I happen to be the author of [Moxie](http://www.moxiemocks.org/).  I wrote Moxie after I spent nearly three years on a project that used all four other libraries in this comparison, and got annoyed with all of them one reason or another.  So I'm somewhat biased.

# The Libraries

### JMock 2

[JMock 2](http://www.jmock.org/) has had the most attention of the current crop of Java mocking libraries - it has had a large user base, and has been mentioned in numerous books (including _[Growing Object-Oriented Software Guided by Tests](http://www.growing-object-oriented-software.com/)_, written by JMock's authors).  It was notable for having a mocking syntax designed to make sense as a domain-specific language (DSL).  Proponents say its DSL is the most expressive API of any mocking library; critics say its mocking syntax as commonly used, involving the creation of an anonymous inner class per test, is excessively verbose.
...
[JMock 1](http://www.jmock.org/jmock1.html) was my introduction to mocking around 2003.  JMock 1 is generally considered a legacy library; new projects should use one of the other libraries in this article.
...
Along with JMock 2, [EasyMock](http://www.easymock.org/) is the other established mocking library for Java 1.5; its syntax is somewhat more concise than that of the two JMocks (especially JMock 2).
...
[Mockito](http://www.mockito.org/) is similar in API style to EasyMock (it started out as a fork of EasyMock), but takes a different approach to mocking: rather than setting up an exhaustive set of expectations ahead of time, you instead stub out any necessary behavior on your mocks, then after calling the code under test verify that the relevant methods on your mock objects were called.
...
[Mockachino](https://github.com/krka/mockachino) is a newish Java mocking framework.  It takes Mockito's verification-driven approach to testing.
...
Our example code will use mocks to test `ROT13List` - a simple class that implements `List<String>`, transforming the strings using the [ROT13](http://en.wikipedia.org/wiki/ROT13) algorithm when storing/retrieving them from an underlying `List<String>`.
...
This shows JMock 2.5.1 as used with JUnit 4.  The JMock web site also has [examples](http://www.jmock.org/getting-started.html) for using JMock 2 stand-alone and with JUnit 3.
...
For those unfamiliar with this Java syntax, the code within the double braces is a Java [instance initializer](http://java.sun.com/docs/books/jls/third_edition/html/classes.html#246032) block - essentially a no-arg constructor for the anonymous subclass created in this method.
...
JMock 2's fuzzy match syntax largely revolves around [Hamcrest](http://github.com/hamcrest) matchers; the `with()` method can be used to match parameters against an arbitrary `Matcher`.
